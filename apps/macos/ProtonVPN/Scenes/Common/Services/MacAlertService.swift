...
extension MacAlertService: CoreAlertService {
    ...
    private func show(_ alert: SystemExtensionTourAlert) {
        let viewModel = SystemExtensionGuideViewModel(extensionsCount: alert.extensionsCount, alertService: self, isTimeToClose: alert.isTimeToClose, acceptedHandler: alert.continueHandler)
