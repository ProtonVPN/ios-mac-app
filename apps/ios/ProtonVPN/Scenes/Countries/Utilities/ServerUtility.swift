...
class ServerUtility {
    ...
    static func country(in grouping: [CountryGroup], countryCode: String) -> CountryModel? {
        guard let index = countryIndex(in: grouping, countryCode: countryCode) else {
            return nil
        }
        return country(in: grouping, index: index)
    }
